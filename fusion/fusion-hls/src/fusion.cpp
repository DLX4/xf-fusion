...
void buildLaplacianPyramids(xf::Mat<TYPE, ROWS, COLS, NPC1>& src, xf::Mat<TYPE, ROWS, COLS, NPC1>& pyr0, xf::Mat<TYPE, ROWS/2, COLS/2, NPC1>& pyr1, xf::Mat<TYPE, ROWS/4, COLS/4, NPC1>& pyr2, xf::Mat<TYPE, ROWS/8, COLS/8, NPC1>& pyr3, xf::Mat<TYPE, ROWS/16, COLS/16, NPC1>& pyr4) {
    ...
    // 往下构造本层高斯金字塔 第1层
    pyrDownUpDown(pyr0, pyr1);
    // 往下构造本层高斯金字塔 第2层
    pyrDownUpDown(pyr1, pyr2);
    // 往下构造本层高斯金字塔 第3层
    pyrDownUpDown(pyr2, pyr3);
    // 往下构造本层高斯金字塔 第4层
    pyrDownUpDown(pyr3, pyr4);
    ...
    xf::subtract<XF_CONVERT_POLICY_SATURATE, TYPE, ROWS, COLS, NPC1>(pyr0, expend1, pyr0);

    // 第2层
    xf::Mat<TYPE, ROWS/2, COLS/2, NPC1> expend2(pyr1.rows, pyr1.cols);// upscale 2x
    xf::pyrUp(pyr2, expend2);
    xf::subtract<XF_CONVERT_POLICY_SATURATE, TYPE, ROWS/2, COLS/2, NPC1>(pyr1, expend2, pyr1);

    // 第3层
    xf::Mat<TYPE, ROWS/4, COLS/4, NPC1> expend3(pyr2.rows, pyr2.cols);// upscale 2x
    xf::pyrUp(pyr3, expend3);
    xf::subtract<XF_CONVERT_POLICY_SATURATE, TYPE, ROWS/4, COLS/4, NPC1>(pyr2, expend3, pyr2);

    // 第4层
    xf::Mat<TYPE, ROWS/8, COLS/8, NPC1> expend4(pyr3.rows, pyr3.cols);// upscale 2x
    xf::pyrUp(pyr4, expend4);
    xf::subtract<XF_CONVERT_POLICY_SATURATE, TYPE, ROWS/8, COLS/8, NPC1>(pyr3, expend4, pyr3);
}
